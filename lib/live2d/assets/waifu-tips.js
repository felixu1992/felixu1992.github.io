...
function loadTipsMessage(result) {
    ...
    window.showWelcomeMessage = function(result) {
        var text;
        if (window.location.pathname == live2d_settings.homePageUrl) {
            var date = new Date();
            var now = date.getHours() + ((date.getMinutes() > 0 || date.getSeconds() > 0) ? 1 : 0);
            if (now > 23 || now <= 4) text = getRandText(result.waifu.hour_tips['t23-4']);
            else if (now > 4 && now <= 8) text = getRandText(result.waifu.hour_tips['t4-8']);
            else if (now > 8 && now <= 12) text = getRandText(result.waifu.hour_tips['t8-12']);
            else if (now > 12 && now <= 14) text = getRandText(result.waifu.hour_tips['t12-14']);
            else if (now > 14 && now <= 17) text = getRandText(result.waifu.hour_tips['t14-17']);
            else if (now > 17 && now <= 19) text = getRandText(result.waifu.hour_tips['t17-19']);
            else if (now > 19 && now <= 21) text = getRandText(result.waifu.hour_tips['t19-21']);
            else if (now > 21 && now <= 23) text = getRandText(result.waifu.hour_tips['t21-23']);
            else text = getRandText(result.waifu.hour_tips.default);
        } else if (window.location.pathname == live2d_settings.categoriesPage) {
            text = result.waifu.path_tips.categories;
        } else if (window.location.pathname == live2d_settings.archivesPage) {
            text = result.waifu.path_tips.archives;
        } else if (window.location.pathname == live2d_settings.tagsPage) {
            text = result.waifu.path_tips.tags;
        } else if (window.location.pathname == live2d_settings.aboutPage) {
            text = result.waifu.path_tips.about;
        } else {
